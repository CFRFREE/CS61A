...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    alist = [st for st in paragraphs if select(st)]
    if len(alist) > k:
        return alist[k]
    else:
        return ""
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def fun(st):
        alist = split(lower(remove_punctuation(st)))
        for it in subject:
            if it in alist:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for st in word_list:
        if st == typed_word:
            return st
        if ans == "" or diff_function(typed_word, st, limit) < diff_function(
            typed_word, ans, limit
        ):
            ans = st
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:
        return min(limit + 1, len(source) + len(typed))
    if typed[0] == source[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    limit: a number representing an upper boundnumber of edits
    >>> big_limit = 10
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    n = len(timestamps_per_player)
    m = len(words)
    times = []
    for item in timestamps_per_player:
        alist = []
        for i in range(1, m + 1):
            alist.append(item[i] - item[i - 1])
        times.append(alist)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if mini_pos == None or time(match, j, i) < time(match, mini_pos, i):
                mini_pos = j
        ans[mini_pos].append(get_word(match, i))
    return ans
    # END PROBLEM 10
